...
def GetPoissonizedGraph(histo):
    ...
    j=0
    for i in xrange(1,histo.GetNbinsX()+1):  ## ignoring under/overflow
...
def GetPoissonizedRatioGraph(h_data,h_mc):
    graph = ROOT.TGraphAsymmErrors()
    j=0
    for i in xrange(1,h_data.GetNbinsX()+1):   ## ignoring under/overflow
...
def GetMCRatioBand(h_mc):
    graph = ROOT.TGraphAsymmErrors()
    j=0
    for i in xrange(1,h_mc.GetNbinsX()+1):   ## ignoring under/overflow
        nmc   = h_mc.GetBinContent(i)
